<template>
  <div class="div">
            ...
            <div class="sc-table">
              <div class="tool-bar el-row" style="padding-bottom: 20px">
                <div class="actions el-col el-col-8">
                  <router-link :to="{ name: 'admin.blog.blog.create' }">
                    <el-button type="primary">
                      <i class="el-icon-edit"></i>
                      {{ trans("blogs.create blog') }}
                    </el-button>
                  </router-link>
                </div>
              ...
              >
                ...
                <el-table-column :label="trans('core.table.actions')" prop="actions">
                  <template slot-scope="scope">
                    <el-button-group>
                      <edit-button :to="{ name: 'admin.blog.blog.edit', params: { blogId: scope.row.id } }"></edit-button>
    ...
    <button v-show="false" v-shortkey="['c']" @shortkey="pushRoute({ name: 'admin.blog.blog.create' })"></button>
  </div>
</template>
...
export default {
  ...
  methods: {
    queryServer(customProperties) {
      ...
      axios.get(route("api.blog.blog.indexServerSide", { ...properties, ...customProperties })).then((response) => {
    ...
    goToEdit(scope) {
      this.pushRoute({ name: "admin.blog.blog.edit", params: { blogId: scope.row.id } });
    },
    editRoute(scope) {
      return route("admin.blog.blog.edit", [scope.row.id]);
    },
  },
};
</script>
