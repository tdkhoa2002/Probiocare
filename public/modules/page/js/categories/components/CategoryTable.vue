<template>
  <div class="div">
            ...
            <div class="sc-table">
              <div class="tool-bar el-row" style="padding-bottom: 20px">
                <div class="actions el-col el-col-8">
                  <router-link :to="{ name: 'admin.post.category.create' }">
                    <el-button type="primary">
                      <i class="el-icon-edit"></i>
                      {{ trans("pages.create category") }}
                    </el-button>
                  </router-link>
                </div>
              ...
              >
                ...
                <el-table-column :label="trans('core.table.actions')" prop="actions">
                  <template slot-scope="scope">
                    <el-button-group>
                      <edit-button :to="{ name: 'admin.post.category.edit', params: { categoryId: scope.row.id } }"></edit-button>
    ...
    <button v-show="false" v-shortkey="['c']" @shortkey="pushRoute({ name: 'admin.post.category.create' })"></button>
  </div>
</template>
...
export default {
  ...
  methods: {
    queryServer(customProperties) {
      ...
      axios.get(route("api.post.category.indexServerSide", { ...properties, ...customProperties })).then((response) => {
    ...
    goToEdit(scope) {
      this.pushRoute({ name: "admin.post.category.edit", params: { categoryId: scope.row.id } });
    },
    editRoute(scope) {
      return route("admin.post.category.edit", [scope.row.id]);
    },
  },
};
</script>
