import { convertToOHLC, makeApiRequest } from "./helpers.js";
...
export default class DataFeed {
    ...
    async getBars(symbolInfo, resolution, periodParams, onHistoryCallback, onErrorCallback) {
        ...
        try {
            ...
            const bars = convertToOHLC(res.data.tradeHistories.map((item) => ({ time: new Date(item.created_at).getTime() / 1000, price: item.price })), this.TIME_LIST_MAP_NUMBER[resolution])
                .sort((a, b) => {
                    b.time - a.time;
                })
    ...
    async subscribeBars(symbolInfo, resolution, onRealtimeCallback, subscriberUID, onResetCacheNeededCallback) {
        ...
        channel.bind("HookTradeMarket", function (data) {
            ...
            if (this._lastBar) {
                const currentCandleTime = Math.floor(nnew Date().toISOString());
                ...
                this._lastBar = {
                    time: Math.floor(nnew Date().toISOString()),
                    open,
                    close,
                    high,
                    low,
