<template>
  <b-overlay :show="showOverlayPaymentReceived" rounded="sm">
    <b-modal
      v-model="showModalPaymentReceived"
      hide-footer
    ...
    >
      ...
      <div class="d-flex justify-content-center">
        ...
        <button class="btn btn-primary ms-2" @click="submitPaymentReceived">
          Confirm
        </button>
      </div>
    </b-modal></b-overlay
...
export default {
  ...
  data() {
    return {
      showOverlayPaymentReceived: false,
      checkSubmit: false,
  ...
  methods: {
    submitPaymentReceived() {
      this.showOverlayPaymentReceived = true;
      if (this.checkSubmit == false) {
        ...
        this.showOverlayPaymentReceived = false;
        return false;
      }
      let url = "/p2p/agent/payment-received/" + this.orderId;
      Axios.post(url, { payment_method_id: this.paymentMethodId })
        .then((response) => {
          this.showOverlayPaymentReceived = false;
          this.closeModal();
