                  ...
                  <div class="col-12">
                    <div class="d-flex mb-3">
                      <div class="d-flex flex-column me-5">
                        ...
                        <div class="fs-5">{{ ads.fixed_price }} {{ fiatCurrency.code }}</div>
                      </div>
                    ...
                    <div class="form-item">
                      ...
                      <div class="form-check mb-2">
                        ...
                        <label class="fw-light" for="flexCheckHolding"
                          >Hoding more than {{ assetCurrency.code }}</label
                        >
                      </div>
                      <input
                        class="input text-center mb-2"
            ...
            <div class="d-flex flex-nowrap mb-1">
              ...
              <div class="" style="width: 66%">
                ...
                <div class="fw-light" v-if="ads.require_holding">
                  Holding more than {{ ads.require_holding_amount }} {{ assetCurrency.code }}
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    submitForm() {
      ...
      let url = "/p2p/agent/update/"+ this.adsId;
      this.form
        .post(url)
        .then((response) => {
    ...
    findAds() {
      let url = "/p2p/agent/findAds/" + this.adsId;
      Axios.get(url)
        .then((response) => {
          if (response.data.error === false) {
            this.ads = response.data.data.ads;
            this.selectedPaymentMethods = response.data.data.paymentMethodSelected;
            this.fiatCurrency= response.data.data.fiat_currency;
            this.assetCurrency= response.data.data.asset_currency;
            this.changeType(this.ads.type);
          }
        })
        .catch((error) => {
          console.log(error);
