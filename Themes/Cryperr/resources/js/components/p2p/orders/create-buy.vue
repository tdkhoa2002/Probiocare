    ...
    <div class="row g-2 g-md-4" v-if="ads != null">
      <div class="col-12 col-md-8">
        <div class="secondary-card">
          <div class="card-title fs-5 mb-4" v-if="ads.type == 'SELL'">
            BUY {{ asset_currency.code }} with {{ fiat_currency.code }}
          </div>
          <div class="mb-4">
          ...
          <div class="form d-flex flex-column justify-content-between h-100">
            <div>
              <div class="form-item">
                ...
                <div class="input-group bg-light">
                  ...
                  <div class="input-suffix">
                    <div class="ms-2" v-if="ads.type == 'SELL'">
                      {{ fiat_currency.code }}
                    </div>
                  </div>
                </div>
              ...
              <div class="d-flex justify-content-center">
                ...
                <button class="btn btn-green w-100 ms-2" @click="submitForm">
                  BUY
                </button>
              </div>
            </div>
...
export default {
  ...
  methods: {
      ...
      Axios.get(url)
        .then((response) => {
          if (response.data.error === false) {
            ...
            this.asset_currency = response.data.data.asset_currency;
            this.paymentMethodAds = response.data.data.paymentMethodSelected;
          }
        })
        .catch((error) => {
          console.log(error);
    ...
    onChangePay(e) {
      ...
      console.log(price);
      console.log(this.order.pay);
      this.order.receive = this.$helpers.round(this.order.pay / price, 2);
    },
    onChangeReceive(e) {
      const value = e.target.value;
      this.order.receive = +value;
      const price = this.ads.fixed_price;
      this.order.pay = this.$helpers.round(this.order.receive * price, 2);
    },
    submitForm() {
