<template>
  <div class="div">
            ...
            <div class="sc-table">
              <div class="tool-bar el-row" style="padding-bottom: 20px">
                <div class="actions el-col el-col-8">
                  <router-link :to="{ name: 'admin.staking.package.create' }">
              ...
              >
                ...
                <el-table-column :label="trans('core.table.actions')" prop="actions" fixed="right">
                  <template slot-scope="scope">
                    <el-button-group>
                      <edit-button :to="{ name: 'admin.staking.package.edit', params: { packageId: scope.row.id } }"></edit-button>
    ...
    <button v-show="false" v-shortkey="['c']" @shortkey="pushRoute({ name: 'admin.staking.package.create' })"></button>
  </div>
</template>
...
export default {
  ...
  methods: {
    queryServer(customProperties) {
      ...
      axios.get(route("api.staking.package.indexServerSide", { ...properties, ...customProperties })).then((response) => {
    ...
    goToEdit(scope) {
      this.pushRoute({ name: "admin.staking.package.edit", params: { packageId: scope.row.id } });
    },
    editRoute(scope) {
      return route("admin.staking.package.edit", [scope.row.id]);
    }
  },
};
</script>
