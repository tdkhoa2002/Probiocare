...
class TargetPrice extends Command
{
    /**
     ...
     * @var string
     */
    protected $description = 'This is the target price.';
    ...
    {
        try {
            $targetPrices = $this->targetPriceRepository->getByAttributes([
                'status' => 1
            ]);
            foreach ($targetPrices as $targetPrice) {
                if(Carbon::now()->toDateTimeString() >= $targetPrice->schedule) {
                    $pair = $targetPrice->market;
                    $customer = $targetPrice->customer;
                    if ($pair) {
                    ...
                    $this->targetPriceRepository->update($targetPrice, [
                        'status' => 0
                    ]);
                }
            }
