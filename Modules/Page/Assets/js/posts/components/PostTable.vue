<template>
  <div class="div">
            ...
            <div class="sc-table">
              <div class="tool-bar el-row" style="padding-bottom: 20px">
                <div class="actions el-col el-col-8">
                  ...
                  <router-link :to="{ name: 'admin.post.post.create' }">
                    <el-button type="primary">
                      <i class="el-icon-edit"></i>
                      {{ trans("pages.create post") }}
                    </el-button>
                  </router-link>
                </div>
              ...
              >
                ...
                <el-table-column :label="trans('core.table.actions')" prop="actions">
                  <template slot-scope="scope">
                    <el-button-group>
                      <edit-button :to="{ name: 'admin.post.post.edit', params: { pageId: scope.row.id } }"></edit-button>
    ...
    <button v-show="false" v-shortkey="['c']" @shortkey="pushRoute({ name: 'admin.post.post.create' })"></button>
  </div>
</template>
...
export default {
  ...
  methods: {
    queryServer(customProperties) {
      ...
      axios
        .get(
          route("api.post.post.indexServerSide", {
            ...properties,
            ...customProperties,
          })
        )
    ...
    handleExtraActions(action) {
      ...
      axios
        .get(route("api.page.page.mark-status", { action, pageIds: JSON.stringify(pageIds) }))
        .then((response) => {
          this.$message({
            type: "success",
          ...
          this.data
            .filter((page) => pageIds.indexOf(page.id) >= 0)
            .map((p) => {
              const page = p;
    ...
    goToEdit(scope) {
      this.pushRoute({
        ...
        params: { postId: scope.row.id },
      });
    },
    editRoute(scope) {
      return route("admin.post.post.edit", [scope.row.id]);
    },
  },
};
</script>
