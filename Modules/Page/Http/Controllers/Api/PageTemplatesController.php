...
class PageTemplatesController extends Controller
{
    ...
    {
        return response()->json($this->getTemplates($request->directory));
    }

    private function getTemplates($directory)
    {
        $path = $this->getCurrentThemeBasePath();

        $templates = [];

        foreach ($this->finder->excluding(config('asgard.page.config.template-ignored-directories', []))->allFiles($path . '/views/' . $directory) as $template) {
