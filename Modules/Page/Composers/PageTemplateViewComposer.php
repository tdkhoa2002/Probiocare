...
class PageTemplateViewComposer
{
    /**
     * @var ThemeManager
     */
    ...
    {
        $view->with('page_templates', $this->getPageTemplates());
    }
    ...
    private function getPageTemplates()
    {
        ...
        $templates = [];
        foreach ($this->finder->excluding(config('asgard.page.config.template-ignored-directories', []))->allFiles($path . '/views/page') as $template) {
            ...
            if ($this->hasSubdirectory($relativePath)) {
                $templates['page.' . str_replace('/', '.', $relativePath) . '.' . $file] = 'page.' . $templateName;
            } else {
                $templates['page.' . $file] = 'page.' . $templateName;
            }
        }
        return $templates;
    }
