...
export default {
  ...
  methods: {
    queryServer(customProperties) {
      ...
      axios.get(route("api.loyalty.order.indexServerSide", { ...properties, ...customProperties })).then((response) => {
    ...
    getReportTotal() {
      axios.get(route("api.loyalty.report.reportTotal")).then((response) => {
    ...
    goToDetail(scope) {
      this.pushRoute({ name: "admin.loyalty.order.detail", params: { orderId: scope.row.id } });
    },
  },
};
</script>
