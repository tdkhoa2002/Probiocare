<template>
  <div class="div">
            ...
            <div class="sc-table">
              <div class="tool-bar el-row" style="padding-bottom: 20px">
                <div class="actions el-col el-col-8">
                  <router-link :to="{ name: 'admin.cryperrswap.currency.create' }">
              ...
              >
                ...
                <el-table-column :label="trans('currencies.table.name')" prop="name">
                  <template slot-scope="scope">
                ...
                <el-table-column :label="trans('currencies.table.coin')" prop="coin">
                  <template slot-scope="scope">
                    {{ scope.row.coin }}
                  </template>
                </el-table-column>
                <el-table-column :label="trans('currencies.table.network')" prop="network">
                  <template slot-scope="scope">
                    {{ scope.row.network }}
                  </template>
                </el-table-column>
                <el-table-column :label="trans('currencies.table.priority')" prop="priority">
                  <template slot-scope="scope">
                    {{ scope.row.priority }}
                  </template>
                </el-table-column>
                ...
                <el-table-column :label="trans('core.table.actions')" prop="actions">
                  <template slot-scope="scope">
                    <el-button-group>
                      <edit-button :to="{ name: 'admin.cryperrswap.currency.edit', params: { currencyId: scope.row.id } }"></edit-button>
    ...
    <button v-show="false" v-shortkey="['c']" @shortkey="pushRoute({ name: 'admin.cryperrswap.currency.create' })"></button>
  </div>
</template>
...
export default {
  ...
  methods: {
    queryServer(customProperties) {
      ...
      axios.get(route("api.cryperrswap.currency.indexServerSide", { ...properties, ...customProperties })).then((response) => {
    ...
    goToEdit(scope) {
      this.pushRoute({ name: "admin.cryperrswap.currency.edit", params: { currencyId: scope.row.id } });
    },
    editRoute(scope) {
      return route("admin.cryperrswap.currency.edit", [scope.row.id]);
    },
    syncAddress(currencyId) {
      this.loading = true;
      axios
        .get(
          route("api.cryperrswap.currency.syncAddress", {
            currency: currencyId,
          })
        )
        .then((response) => {
