...
class RegisterSettingSidebar extends AbstractAdminSidebar
{
    ...
    public function extendWith(Menu $menu)
    {
        ...
        $menu->group(trans('workshop::workshop.appearance'), function (Group $group) {
            $group->item(trans('setting::themeoptions.title.themeoptions'), function (Item $item) {
                $item->icon('fa fa-cog');
                $item->weight(20);
                $item->route('admin.themeoption.themeoptions.index');
                $item->authorize(
                    $this->auth->hasAccess('themeoption.themeoptions.index')
                );
            });
        });
